...
export default function WalletPage() {
  ...
  useEffect(() => {
    // Check if user is logged in and load wallet data
    const loadWallet = async () => {
      try {
        ...
        setTotalWithdrawn(wallet.total_withdrawn);
      ...
      <main className="min-h-screen bg-linear-to-br from-blue-50 via-white to-purple-50 py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div
            ...
            className="bg-linear-to-br from-blue-600 to-purple-700 text-white rounded-3xl shadow-2xl p-8 mb-12"
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              >
                ...
                <div className="w-full h-2 bg-white/20 rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${(walletBalance / 500) * 100}%` }}
                    transition={{ duration: 1 }}
                    className="h-full bg-linear-to-r from-green-400 to-blue-400"
                  />
                </div>
                <p className="text-xs text-blue-100 mt-2">
                  Level: Silver (₹{(500 - walletBalance).toFixed(2)} to Gold)
                </p>
              </motion.div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              <motion.button
                ...
                className="bg-green-500 hover:bg-green-600 text-white px-8 py-3 rounded-lg font-bold transition-all"
              >
                🏦 Withdraw to Bank
              </motion.button>
              <Link href="/upload">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="bg-blue-400 hover:bg-blue-300 text-white px-8 py-3 rounded-lg font-bold transition-all"
                >
                  📤 Upload More Data
                </motion.button>
              </Link>
              <Link href="/history">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="bg-purple-400 hover:bg-purple-300 text-white px-8 py-3 rounded-lg font-bold transition-all"
                >
                  📋 Upload History
                </motion.button>
              </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              <p className="text-gray-700 mb-4">
                You&apos;re ₹254.50 away from Gold membership with 2x rewards!
              </p>
          ...
          >
            ...
            <div className="flex gap-4">
              <motion.button
                ...
                className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-900 font-bold hover:bg-gray-50"
              >
                Cancel
              </motion.button>
              <motion.button
